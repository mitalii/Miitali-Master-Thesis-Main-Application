<template>
  <div onload="startVideo" class="w-screen h-screen bg-cyan-500 relative">
    <div class="absolute">
      <div class="flex flex-col justify-center">
        <video id="video-recorder" :src="camera_stream" class="rounded-lg shadow hidden" width="240" autoplay></video>
        <button id="start-camera" class="px-4 py-2 bg-white rounded m-2">Start Camera</button>
        <button id="start-record" class="hidden px-4 py-2 bg-white rounded m-2">Start Recording</button>
        <button id="stop-record" class="hidden px-4 py-2 bg-white rounded m-2">Stop Recording</button>
        <a id="download-video" download="test.webm" class="hidden px-4 py-2 bg-white rounded m-2">Download Video</a>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    // MoodNumber(){
    //   fetch(
    //             "https://mitali-sharma-default-rtdb.europe-west1.firebasedatabase.app/mitali.json",
    //             {
    //               method: "POST",
    //               headers: {
    //                 "Content-Type": "application/json"
    ...
    setMoodNum(value){
      ...
      // }
      if(value == '1'){
        this.articles.allArticles.push(...this.happyData);
        this.articles.allArticles.push(...this.sadData);
  ...
  mounted() {
    ...
    let video = document.querySelector("#video-recorder");
